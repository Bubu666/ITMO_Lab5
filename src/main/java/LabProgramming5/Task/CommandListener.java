...
/**
 * Предназначен для считывания и обработки пользователььских действий над объектом класса {@code Storage}
 */
public class CommandListener {
    ...
    /**
     * Переменная хранилища, над которым будут совершаться пользовательские действия
     */
            ...
            for (Method m : Storage.class.getDeclaredMethods()) {
                if (m.isAnnotationPresent(HelpInfo.class)) {
                    commands.put(m.getName(),
                            Class.forName("LabProgramming5.StorageCommands." + castToCommandClass(m.getName())).getConstructors()[0]);
                }
            }
    ...
    /**
     * Принимает ссылку на хранилище, для которого будут считываться пользовательские команды
     *
    ...
    public void handle(String[] command) {
        ...
        switch (m.command) {
            ...
            case "add": {
                ...
                if (m.args.length > 0) {
                    ...
                    if (m.args[0].equalsIgnoreCase("RANDOM")) {
                        if (m.args.length == 2 && m.args[1].matches("\\d+")) {

                            try {
                                int num = Integer.parseInt(m.args[1]);
                                for (int i = 0; i < num; ++i) {
            ...
            case "execute_script": {
                if (m.args.length == 1) {
                    storage.apply(new ExecuteScript(storage, m.args[0]));
                } else {
                    output.println(wrongArguments);
                }
                return;
            ...
            case "enter_new_init_file": {
                if (m.args.length == 1) {
                    storage.apply(new EnterNewInitFile(storage, m.args[0]));
                } else {
                    output.println(wrongArguments);
                }
                return;
            ...
            default: {
                try {
                    Command cmd = (Command) (m.constructor).newInstance(storage);
